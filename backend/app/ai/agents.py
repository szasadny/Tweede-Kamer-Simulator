...
class ParliamentaryAgent:
    ...
    async def generate_debate_entry(self, db: Session, member_data: Dict[Any, Any], proposal_data: Dict[Any, Any]) -> str:
        """
        Generate a debate entry for a parliament member
        """
        prompt = get_debate_prompt(db, member_data, proposal_data)
        response = await self.client.generate_response(prompt, temperature=0.8)
        return response
    
    async def generate_vote(self, db: Session, member_data: Dict[Any, Any], proposal_data: Dict[Any, Any], debate_summary: str) -> VoteType:
        """
        Generate a vote for a parliament member
        """
        prompt = get_voting_prompt(db, member_data, proposal_data, debate_summary)
    ...
    async def generate_debate_summary(self, db: Session, debate_entries: List[Dict[Any, Any]]) -> str:
        """
        Generate a summary of a debate
        """
        prompt = get_debate_summary_prompt(db, debate_entries)
